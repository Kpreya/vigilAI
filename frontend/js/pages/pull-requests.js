    ...
    return `
      <form id="new-fix-form" class="space-y-4">
        <div>
          ...
          <select name="incidentId" required class="w-full px-3 py-2 border border-black font-mono text-xs focus:outline-none focus:ring-0 focus:border-black focus:shadow-brutal-sm transition-all bg-white cursor-pointer" size="8">
            <option value="" disabled selected>Select an incident to analyze and resolve...</option>
            ${this.openIncidents.map(inc => `<option value="${inc.id}" class="py-1 px-2 hover:bg-blue-50">${this.escapeHtml(inc.title)}</option>`).join('')}
          </select>
          ${this.openIncidents.length === 0 ? '<p class="text-xs text-red-600 font-mono mt-1">No open incidents to fix.</p>' : ''}
          <p class="text-xs text-slate-500 font-mono mt-2 flex gap-1 items-center">
             <span class="material-symbols-outlined text-[14px]">auto_awesome</span> VigilAI will use Claude 3 and your GitHub integration to generate a PR for this issue automatically.
          </p>
        </div>
      </form>
    `;
