...
export async function POST(
  ...
  try {
    ...
    let functionName = ""; // Declare at broader scope for use in AI prompt
    ...
    if (githubToken && cleanGithubOwner && cleanGithubRepo) {
        try {
            ...
            if (incident.stackTrace) {
                // Split stack trace into lines and look for actual source files
                const lines = incident.stackTrace.split('\n');
                
                for (const line of lines) {
                    // Skip webpack/Next.js generated chunks and node_modules
                    if (line.includes('/_next/static/') || 
                        line.includes('/node_modules/') ||
                        line.includes('webpack-internal:') ||
                        line.includes('._.js') ||
                        line.includes('chunks/')) {
                        
                        // But try to extract function name for later search
                    ...
                    const patterns = [
                        /at\s+(?:\w+\s+)?\(([a-zA-Z0-9_\-\.\/]+\.(?:tsx|ts|jsx|js|py|go|java|rb)):\d+:\d+\)/,
                        /at\s+([a-zA-Z0-9_\-\.\/]+\.(?:tsx|ts|jsx|js|py|go|java|rb)):\d+:\d+/,
                        /([a-zA-Z0-9_\-\.\/]+\.(?:tsx|ts|jsx|js|py|go|java|rb)):\d+:\d+/,
                        /\(([a-zA-Z0-9_\-\.\/]+\.(?:tsx|ts|jsx|js|py|go|java|rb)):\d+:\d+\)/,
                    ];
                    ...
                    for (const pattern of patterns) {
                        ...
                        if (match && match[1]) {
                            ...
                            console.log(`Extracted path from stack trace: ${extractedPath} (from line: ${line.trim()})`);
                            break;
                        }
                    }
                ...
                // Fallback: if no path found, try the old simple pattern
                if (!extractedPath) {
                    const match = incident.stackTrace.match(/([a-zA-Z0-9_\-\.\/]+\.(?:tsx|ts|jsx|js|py|go|java|rb|css|html))/);
                    if (match && match[1] && !match[1].includes('/_next/') && !match[1].includes('chunks/')) {
                        extractedPath = match[1];
                        console.log(`Extracted path from stack trace (fallback): ${extractedPath}`);
                    }
                }
                
                // If still no path but we have a function name, we can mention it in the diagnosis
                if (!extractedPath && functionName) {
                    console.log(`⚠️ No source file found, but detected function: ${functionName}`);
                }
            }
            ...
            if (!targetFilePath) {
                ...
                console.error(`Extracted path: ${extractedPath || 'NONE'}`);
                ...
                try {
                    ...
                    if (Array.isArray(data)) {
                        repoContext = `Could not locate specific file. The stack trace contains webpack/Next.js compiled files which don't exist in the repository. Project root contains: ${data.map(f => f.name).join(', ')}`;
                    }
                } catch (e) {
...
${!targetFilePath && functionName ? `
NOTE: The stack trace contains compiled/bundled files. The error likely originates from a function called "${functionName}".
Please provide a general diagnosis and suggested fix pattern that can be applied to the source code containing this function.
` : ''}

TASK:
1. Analyze the error and identify the root cause
2. ${targetFilePath ? 'Generate a COMPLETE, WORKING replacement for the entire source file that fixes the bug' : 'Provide a detailed diagnosis and fix strategy'}
...
SUGGESTED_FIX:
${targetFilePath ? '<complete raw source code file - NO markdown formatting>' : '<detailed fix instructions and code patterns>'}
        `.trim();
    ...
    if (octokit && cleanGithubOwner && cleanGithubRepo) {
        ...
        try {
            ...
            if (!targetFilePath) {
                throw new Error(`Cannot create PR: Failed to locate source file in repository. The stack trace contains compiled/bundled files (like webpack chunks) that don't exist in the source code. This typically happens with Next.js or other bundled applications. To fix this, ensure your error tracking captures the original source file paths, or manually specify the file to fix.`);
            }
